...
TRAIN tech=FORGY
;
SAVE outstat=WORK.Clus2_OUTSTAT outmean=EMWS1.Clus2_OUTMEAN;
code file="C:\Users\skunt\Downloads\HU.Exploratory Analysis\HU.ClusterAnalysis\Workspaces\EMWS1\Clus2\DMVQSCORECODE.sas"
group=Clus2
;
run;
quit;
...
%mend ;
*------------------------------------------------------------*;
* Clus2: Determining the number of clusters;
*------------------------------------------------------------*;
proc cluster data=EMWS1.Clus2_OUTMEAN method=WARD pseudo outtree=EMWS1.Clus2_CLUSSEED
;
var %dmvqvars;
copy _SEGMENT_;
run;
quit;
proc sort data =EMWS1.Clus2_CLUSSEED out=WORK._SEED_;
by _ncl_;
where (_ccc_ > -99999.0);
run;
data WORK._SEED2_;
